...
public sealed class SystemCommandSpawnerIntegrationTests
{
    [Fact]
        ...
        try
        {
            ...
            await WaitUntilAsync(
                () => Directory.EnumerateFiles(toolDirectory, "*.log").Any(path =>
                    File.ReadAllText(path).Contains("newest-token", StringComparison.Ordinal)),
        ...
        Func<string, bool> predicate)
    {
        ...
        await WaitUntilAsync(() =>
        {
            ...
            foreach (var path in Directory.EnumerateFiles(directory, "*.log"))
            {
                ...
                {
                    // The asynchronous output handlers may be flushing this file.
                }
            }
