...
public sealed class ConfigurationStoreTests
{
    ...
    public async Task Round_trip_persists_configuration()
    {
        ...
        {
            Settings = AppConfiguration.CreateDefault().Settings with { LogRetentionRuns = 25 }
        };
    ...
    public async Task Save_uses_atomic_replace_and_keeps_one_backup()
    {
        ...
        await store.SaveAsync(AppConfiguration.CreateDefault() with
        {
            Settings = AppConfiguration.CreateDefault().Settings with { LogRetentionRuns = 20 }
        });
        ...
        Assert.DoesNotExist(Path.Combine(directory.Path, "config.json.tmp"));
    }
    ...
    {
        public string Path { get; } = System.IO.Path.Combine(System.IO.Path.GetTempPath(), "ngr-launcher-tests", Guid.NewGuid().ToString("N"));
